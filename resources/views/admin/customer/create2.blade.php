...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('admin.customer.store2') }}" method="POST" id="customerForm" novalidate>
                @csrf
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="text" name="kode_pos" class="form-control" placeholder="60111" maxlength="5" minlength="5" required>
                    </div>
                </div>
                ...
                <div class="mt-5 border-top pt-3 text-end">
                    ...
                    <button type="submit" class="btn btn-info text-white px-4">Simpan Data (Path)</button>
                </div>
            </form>
        </div>
    </div>
            ...
            if (id) {
                axios.post("{{ route('wilayah.getKota') }}", { id: id, _token: "{{ csrf_token() }}" })
                    .then(res => {
                        ...
                        res.data.data.forEach(item => options += `<option value="${item.id}">${item.name}</option>`);
