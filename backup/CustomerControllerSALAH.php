...
class CustomerController extends Controller
{
    ...
    /**
     * Form Tambah Customer 1 (Simpan foto sebagai BLOB)
     */
    ...
    /**
     * Form Tambah Customer 2 (Simpan foto sebagai File Path)
     */
    ...
    /**
     * Simpan data ke database dengan foto format BLOB (BYTEA di pgsql)
     */
    public function store1(Request $request)
    {
        ...
        try {
            ...
            if (empty($binaryData)) {
                return back()->withErrors(['image' => 'Foto tidak valid']);
            }

            // PostgreSQL bytea kompatibilitas: gunakan hex encoding
            // Ini mengatasi error "invalid byte sequence for encoding UTF8"
    ...
    /**
     * Simpan data ke database dengan foto format File Path
     */
    ...
    //     return redirect()->route('admin.customer.index')->with('success', 'Customer berhasil ditambah (Path)');
    // }
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'kode_pos'      => 'required|string|size:5', // Pastikan 5 karakter
            'id_provinsi'   => 'required|not_in:0',
            'id_kota'       => 'required|not_in:0',
            'id_kecamatan'  => 'required|not_in:0',
            'id_kelurahan'  => 'required|not_in:0',
        ], [
            'not_in' => 'Wilayah :attribute harus dipilih dengan benar.'
        ]);
        ...
        // Logika jika user mengambil foto baru dari Webcam
        if ($request->filled('image')) {
            ...
            // Jika data sebelumnya pakai Path, hapus file lama
            if ($customer->foto_path) {
                Storage::disk('public')->delete($customer->foto_path);
                $dataUpdate['foto_path'] = null;
            }

            // Simpan sebagai BLOB (karena ini yang paling aman untuk update cepat)
            // Gunakan hex encoding untuk PostgreSQL
